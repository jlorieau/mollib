...
# Plot the d_a1d1 vs theta plots
...
for row, subfigure_labels in enumerate(subfigure_groups, 1):
    ...
    for count, values in enumerate(subfigure_labels):
        ...
        # Prepare the data
        xyz = d1a1_theta[title]

        x = np.array([i[0] for i in xyz])  # d_a1d1
        y = np.array([i[1] for i in xyz])  # theta

        # Make the histogram for d_a1d1 vs theta
    ...
    plt.savefig('hbonds/hbond_countour_d_vs_theta_{}.png'.format(row),
                format='PNG', dpi=1200, bbox_inches='tight', pad_inches=0.05)
    plt.savefig('hbonds/hbond_countour_d_vs_theta_{}.svg'.format(row),
                format='SVG', bbox_inches='tight', pad_inches=0.05)

# Plot the d_a1d1 vs phi plots
plt.clf()
plt.cla()
...
for row, subfigure_labels in enumerate(subfigure_groups, 1):
    ...
    # Set the shared y-axis label
    axarr[0].set_ylabel(r'$\phi (deg)', fontsize=axes_fontsize,
                        fontname=axes_font)
    ...
    for count, values in enumerate(subfigure_labels):
        ...
        x = np.array([i[0] for i in xyz])  # d_a1d1
        z = np.array([i[2] for i in xyz])  # phi

        # Make the histogram for d_a1d1 vs theta
        hist2d, z, x = np.histogram2d(z, x, bins=36, range=np.array(
            [phi_hist_range, d_hist_range]))
        hist2d = -1. * np.log(hist2d + 0.1)
    ...
    plt.savefig('hbonds/hbond_countour_d_vs_phi_{}.png'.format(row),
                format='PNG', dpi=1200, bbox_inches='tight', pad_inches=0.05)
    plt.savefig('hbonds/hbond_countour_d_vs_phi_{}.svg'.format(row),
                format='SVG', bbox_inches='tight', pad_inches=0.05)
